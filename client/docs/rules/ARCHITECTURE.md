...
| `services/match/` | **Complete** | 5 svcs w/ tests (calculateSets, determineWinner, formatEvent, applySideSwap, getEventColor) | None |
| `services/dashboard/` | **Complete** | calculateStats with tests | None |
| `services/validation/` | **Complete** | pin.ts, auth.ts, match.ts with tests | None |
| `services/storage/` | **Complete** | authStorage.ts with tests | None |
| `services/errors/` | **Complete** | errorMessages.ts with tests | None |
| `services/url/` | **Complete** | buildScoreboardUrl with tests | None |
| `services/date/` | **Complete** | formatRelativeTime with tests | None |
| `useSocket.ts` | **Model** | 61-line thin wrapper composing 3 focused hooks | None |
| `useSocketConnection` | **Model** | 96 lines, connection lifecycle | None |
| `useSocketState` | **Model** | 74 lines, state from socket events | None |
| `useSocketActions` | **Model** | 100 lines, action emitters | None |
| `usePinSubmission` | **Model** | 93 lines, reusable PIN flow | None |
| `useDashboardStats` | **Model** | Thin wrapper over service | None |
| `useCan` | **Model** | Granular permission check | None |
| `useAuthFlow` | **Model** | Extracted from AuthPage socket handlers | None |
| `AuthContext` | **Complete** | Uses authStorage service | None |
| `QRCodeImage.tsx` | **Model** | Purely presentational, receives URL | None |
| `MatchHistoryTicker` | **Complete** | Uses services/match/formatEvent | None |
| `HistoryDrawer.tsx` | **Complete** | Uses services/date/formatRelativeTime | None |
| `AuthPage.tsx` | **Improved** | Socket events delegated to useAuthFlow hook | None |
| `OwnerDashboardPage` | **Improved** | Uses useTableManagement, useDashboardStats, usePinSubmission (166 lines) | None |
| `useTableManagement` | **Model** | 85-line hook for table CRUD operations | None |
| `useOrientation` | **Improved** | Uses preferencesStorage instead of direct localStorage | None |
| `CoachMark.tsx` | **Improved** | Uses preferencesStorage instead of direct localStorage | None |
| `useScoreboardAuth` | **Removed** | Replaced with usePermissions + useCan | None |
