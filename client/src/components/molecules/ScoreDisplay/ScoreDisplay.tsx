...
export function ScoreDisplay({ 
  ...
  return (
    <motion.div
      ...
      `}
      animate={shouldReduceMotion ? {} : (winner ? { scale: [1, 1.05, 1] } : {})}
    >
      <AnimatePresence mode="wait">
        {shouldReduceMotion ? (
          <span className="font-heading text-[120px] font-bold leading-none text-text-h">
            {score}
          </span>
        ) : (
          <motion.span
  ...
  return (
    <div className="flex items-center justify-center gap-8 landscape:gap-16">
      ...
      <div className="flex flex-col items-center gap-1 px-4">
        <Body className="text-text-muted text-xl landscape:text-2xl">{vsLabel || 'vs'}</Body>
