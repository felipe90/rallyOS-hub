# Admin Table Inventory — Architecture Design
...
Make the **admin** the single source of truth for the tables (mesas/canchas) that
...
Today there are **two parallel, disconnected table worlds**:

- **Tournament / owner courts** (`CourtEventHandler`, event `CREATE_COURT`) — any
  authenticated socket can create an ephemeral table; public list
  `getAllTournamentCourts`; global `MAX_COURTS=50` cap; per-IP rate limit; the
  bracket assigns matches to these tables.
- **Club courts** (`ClubCourtHandler`, event `CLUB_CREATE_COURT`) — only the
  admin creates and manages tables through the lifecycle
...
No one owns the physical inventory; each flow invents its own tables. This
breaks data validity for anything that maps usage to real tables (billing,
analytics, audit).

### Decision: strict mode

- The tournament **must** consume tables from the admin catalog — **no ad‑hoc /
  escape-hatch table creation**. The user accepted this deliberately: a
...
- Admin discipline is assumed: the admin is responsible for keeping the catalog
  current. The system enforces **internal consistency** (every used table exists
...
|---|---|---|---|---|
| **INVENTORY** (existence) | The table exists in the club | Admin (source of truth, durable) | `ACTIVE` · `MAINTENANCE` · `ARCHIVED` | Strict; no hard delete of used tables |
| **AVAILABILITY** (occupancy) | Table is being used right now | Runtime (transient) | `IDLE` · `BUSY` | Derived from inventory, never invented |
| **FLOW** (what happens on the table) | A match OR a club session | Runtime (transient) | per-mode rule engine: tournament `LIVE/...` · club `OCCUPIED/...` | Branches per mode; kept **separate** from inventory |

**Contract**: every table used exists always in the inventory; the inventory is
...
| `HELD` | Committed to a **future** use (general booking only) | an open future booking exists for the court (section 9) — **never** from tournament |
...
1. **Admin — new "Table Inventory" surface**
   - List of club tables with inventory status (`ACTIVE` / `MAINTENANCE` /
     `ARCHIVED`).
   - Actions: **create physical table, rename (display name over stable id),
     set maintenance, archive**.
   - **Force-end** (`INVENTORY_FORCE_END`): the admin can stop ANY live session on
     a table on demand to free it — the emergency release valve (R7). UI shows a
     "End session" action when a table is `BUSY`.
   - **No delete** — used tables archive. UI copy explains: "used tables are
     archived, never deleted."
2. **Owner / tournament — stop creating tables**
   - Remove the ad‑hoc `createCourt` form from the owner dashboard.
   - Tournament **picks tables from the inventory** (picker of `ACTIVE`,
     available tables).
   - Cold start: no `ACTIVE` tables → explicit message. **Provisional copy**
...
3. **Club mode — same flow, different origin**
   - Free practice / match no longer "create" a table; they **occupy an existing
     inventory table**.
   - The mode selector (`free`/`match`, today `ClubSessionConfig`) becomes a
     mode picker over an already-chosen table.
4. **Shared semantics**
...
| Run/assign tournament tables (SELECT) | **Owner** only |
| Occupy a table (club free/match) | **Admin** or **player** via PIN |
| View inventory + availability | All |

Product consequence (decision): **owner tournament now depends on an admin having
configured the inventory.** With no `ACTIVE` tables the owner cannot run a
...
- New (admin): `INVENTORY_FORCE_END` — the admin holds the **stop/force-end
  ...
  traceability. See section 5 flow rules / R7.
...
- New **`InventoryManager`** (a.k.a. `TableInventoryService`) — the **only
  authority over EXISTENCE**. CourtManager consumes it, does not create tables.
...
CourtManager. This replaces the enum+switch anti-pattern (section 7).
...
- Availability must derive from the inventory: a session can only occupy a table
  that exists and is `ACTIVE`. Eliminates ghost-table creation.
...
## 5. Decision rationale: Option A vs Option B
...
- **Option B — table + separable flow**: the table is only
...
## 6. Persistence — performance and problem review
...
|---|---|---|---|
| R1 | **Write amplification**: every point rewrites the whole document including inventory + all sessions | Medium | Decouple inventory file from session file (inventory = low-frequency admin writes; sessions = high-frequency debounced). Separate `TableInventoryStore` from `StateStore`. |
...
- **Hard-delete of used tables** — breaks session-history/cost referential
  integrity.
...
2. Recommended work-unit slicing:
   ...
   - **Update `docs/casos-de-uso.md`** (affected CUs + new inventory/picker/force-end
     CUs) and regenerate the E2E coverage matrix (see section 11).

---

## 9. What's next (post-development): minimal future table booking
...
A booking is simply a record that **a table will be used in the future by a
person**. It is a forward-looking note on an inventory table:

- **Booked**: `table X reserved for person P at datetime D`.
- **Completed**: if P actually shows up at that day/time, the booking is
  ...
  through the normal live flow (the person occupies the table as usual).
...
## 10. Edge cases & decisions (review pass)
...
| E8 | **Future booking** — minimal register; must not set present availability; references `courtId` | ✅ Decided (section 9) |
...
| E14 | **Use-case doc + test matrix impact** — `docs/casos-de-uso.md` CUs and the unit/integration/e2e suites must be updated when events/semantics change (per the doc's Update Protocol) | ✅ Decided — see section 11 |

---

## 11. Use-case & test impact
...
### 11.1 Affected use cases (`docs/casos-de-uso.md`)
