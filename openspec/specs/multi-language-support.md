...
**Cross-cutting concern**: i18n for all client UI text.
...
|---|-----------------|-----------|
| NR1 | **Language switcher UI** — no dropdown, toggle, or settings page for locale selection | Infrastructure + migration only; UI comes in a follow-up |
...
| NR3 | **Runtime language change** — changing locale requires page reload | `i18next` supports runtime change but deferred; initial scope is detect-on-load |
