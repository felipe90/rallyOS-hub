    ...
    checkSideSwap() {
        ...
        if (isFinalSet && !this.state.midSetSwapped) {
            ...
            if (scoreA >= 5 || scoreB >= 5) {
                ...
                logger_1.logger.info('Decisive set midpoint reached: Swapping sides');
            }
        }
    }
    ...
    checkSetWin() {
        ...
        if (hasReachedLimit && hasDifference) {
            ...
            if (this.state.status !== 'FINISHED') {
                ...
                logger_1.logger.debug({ oldSide, newSide: this.state.swappedSides }, 'Set finished. Swapping sides for next set');
            }
            else {
                logger_1.logger.debug('Match finished. Keeping sides as is');
            }
        }
    }
    checkMatchWin() {
