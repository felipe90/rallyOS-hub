...
class TableManager {
    ...
    createTable(name) {
        ...
        logger_1.logger.info({ tableId: id, tableName }, 'Table created');
    ...
    joinTable(tableId, socketId, name, pin) {
        ...
        if (pin && table.pin !== pin) {
            logger_1.logger.warn({ tableId, tableName: table.name }, 'Invalid PIN attempt');
            return false;
        }
        ...
        this.notifyUpdate(table);
        logger_1.logger.info({ tableId, tableName: table.name, playerName: name }, 'Player joined table');
        return true;
    }
    leaveTable(tableId, socketId) {
        ...
        logger_1.logger.info({ tableId, tableName: table.name, playerName: player.name }, 'Player left table');
        this.notifyUpdate(table);
    }
    setReferee(tableId, socketId, pin) {
        ...
        if (existingReferee && existingReferee.socketId !== socketId) {
            logger_1.logger.warn({ tableId, tableName: table.name }, 'Referee already active, rejecting new attempt');
            return false;
        }
        ...
        logger_1.logger.info({ tableId, tableName: table.name, socketId }, 'Referee authenticated');
        this.notifyUpdate(table);
        return true;
    }
    ...
    startMatch(tableId, config) {
        logger_1.logger.info({ tableId, config }, 'startMatch called');
        const table = this.tables.get(tableId);
        if (!table) {
            logger_1.logger.warn({ tableId }, 'startMatch: table not found');
            return null;
        }
        ...
        if (config) {
            logger_1.logger.debug({ tableId }, 'Creating new MatchEngine with config');
            // Preserve table info
        ...
        logger_1.logger.debug({ tableId, status: state?.status }, 'After startMatch, state status');
        this.notifyUpdate(table);
        return state;
    }
    ...
    deleteTable(tableId) {
        ...
        if (deleted) {
            logger_1.logger.info({ tableId }, 'Table deleted');
        }
        return deleted;
    }
    ...
    regeneratePin(tableId) {
        ...
        table.matchEngine.setTableId(table.id, table.name);
        table.status = 'WAITING';
        logger_1.logger.info({ tableId, tableName: table.name, oldRefereeId: oldReferee?.socketId || 'none' }, 'Table reset (kept PIN)');
        this.notifyUpdate(table);
        return table.pin;
    }
    /**
