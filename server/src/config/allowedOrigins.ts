 ...
 * The computed defaults dynamically include HUB_DOMAIN origins (default: rallyos.wifi).
 */
...
export function getHubDomain(): string {
  return process.env.HUB_DOMAIN || 'rallyos.wifi';
}

/**
 * Builds the complete default origin list including dynamic HUB_DOMAIN entries.
 * rallyos.wifi is NEVER hard-coded — it derives from HUB_DOMAIN env var.
 */
